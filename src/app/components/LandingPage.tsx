...
export function LandingPage({ onNavigate }: Props) {
  ...
  return (
    <div className="min-h-screen" style={{ fontFamily: "'Inter',sans-serif", background: "#F5F2EC" }}>
      ...
      {/* HERO */}
      <section id="beranda" style={{ padding: "5rem 1.25rem 5rem", textAlign: "center" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "0.375rem", padding: "0.3rem 0.875rem", borderRadius: 99, background: "#F5ECD9", border: "1px solid #E8D5B0", color: "#A0742A", fontSize: "0.75rem", fontWeight: 600, marginBottom: "1.75rem" }}>
            <Sparkles style={{ width: 11, height: 11 }} />
            Didukung Gemini AI
          </div>

          <h1 style={{ fontSize: "clamp(2rem,5vw,3.25rem)", fontWeight: 700, color: "#1C1917", lineHeight: 1.15, marginBottom: "1.25rem" }}>
            Temukan Judul Skripsi<br />
            <span style={{ color: "#C4933F" }}>Lebih Cepat dengan AI</span>
          </h1>

          <p style={{ fontSize: "1.05rem", color: "#78716C", lineHeight: 1.75, marginBottom: "2.25rem", maxWidth: 480, margin: "0 auto 2.25rem" }}>
          ...
          <button onClick={() => onNavigate("register")}
            style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", padding: "0.75rem 1.75rem", borderRadius: 10, background: "#1C1917", color: "#fff", fontSize: "0.95rem", fontWeight: 600, border: "none", cursor: "pointer", boxShadow: "0 4px 16px rgba(28,25,23,0.18)" }}
          ...
          <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "1.25rem", marginTop: "2rem" }}>
            {["50.000+ judul dibuat","12.000+ mahasiswa aktif","Gratis untuk memulai"].map(t => (
              <div key={t} style={{ display: "flex", alignItems: "center", gap: "0.375rem", fontSize: "0.8rem", color: "#78716C" }}>
      ...
      {/* FEATURES */}
      <section id="fitur" style={{ background: "#FFFFFF", padding: "4.5rem 1.25rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            <p style={{ fontSize: "0.75rem", fontWeight: 700, color: "#C4933F", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "0.5rem" }}>Fitur</p>
            <h2 style={{ fontSize: "1.625rem", fontWeight: 700, color: "#1C1917" }}>Semua yang Anda Butuhkan</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: "1rem" }}>
            {F.map(({ icon: Icon, title, desc }, i) => (
              <div key={i} style={{ padding: "1.375rem 1.5rem", borderRadius: 14, border: "1px solid #E3DDD4", transition: "all 0.2s" }}
                ...
                <p style={{ fontSize: "0.9rem", fontWeight: 600, color: "#1C1917", marginBottom: "0.375rem" }}>{title}</p>
                <p style={{ fontSize: "0.8rem", color: "#78716C", lineHeight: 1.65 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* HOW IT WORKS */}
      <section id="cara-kerja" style={{ background: "#F5F2EC", padding: "4.5rem 1.25rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            <p style={{ fontSize: "0.75rem", fontWeight: 700, color: "#C4933F", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "0.5rem" }}>Cara Kerja</p>
            <h2 style={{ fontSize: "1.625rem", fontWeight: 700, color: "#1C1917" }}>Empat Langkah Sederhana</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "1rem" }}>
            {STEPS.map((s, i) => (
              <div key={i} style={{ background: "#FFFFFF", padding: "1.5rem", borderRadius: 14, border: "1px solid #E3DDD4", textAlign: "center" }}>
                ...
                <p style={{ fontSize: "0.875rem", fontWeight: 600, color: "#1C1917", marginBottom: "0.375rem" }}>{s.t}</p>
                <p style={{ fontSize: "0.78rem", color: "#78716C", lineHeight: 1.6 }}>{s.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* TENTANG */}
      <section id="tentang" style={{ background: "#FFFFFF", padding: "4.5rem 1.25rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            <p style={{ fontSize: "0.75rem", fontWeight: 700, color: "#C4933F", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "0.5rem" }}>Tentang</p>
            <h2 style={{ fontSize: "1.625rem", fontWeight: 700, color: "#1C1917" }}>Tentang SmartTitle AI</h2>
          </div>
          <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center" }}>
            <p style={{ fontSize: "0.95rem", color: "#78716C", lineHeight: 1.8, marginBottom: "2rem" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "1rem" }}>
              {[
                ...
                <div key={i} style={{ padding: "1.5rem", borderRadius: 14, border: "1px solid #E3DDD4", textAlign: "center" }}>
      ...
      {/* CTA */}
      <section style={{ background: "#FFFFFF", padding: "4rem 1.25rem" }}>
        <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
          <div style={{ background: "#1C1917", borderRadius: 18, padding: "2.75rem 2rem" }}>
            <h2 style={{ fontSize: "1.5rem", fontWeight: 700, color: "#FFFFFF", marginBottom: "0.875rem" }}>Mulai Riset Anda Sekarang</h2>
            <p style={{ fontSize: "0.9rem", color: "#A8A29E", lineHeight: 1.7, marginBottom: "1.75rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "row", gap: "0.625rem", justifyContent: "center", flexWrap: "wrap" }}>
              <button onClick={() => onNavigate("register")}
                style={{ padding: "0.625rem 1.5rem", borderRadius: 9, background: "#C4933F", color: "#fff", fontSize: "0.875rem", fontWeight: 600, border: "none", cursor: "pointer" }}
                onMouseEnter={e => (e.currentTarget.style.background = "#B8874A")}
                onMouseLeave={e => (e.currentTarget.style.background = "#C4933F")}>
                Daftar Gratis
              </button>
              <button onClick={() => onNavigate("login")}
                style={{ padding: "0.625rem 1.5rem", borderRadius: 9, background: "transparent", color: "#A8A29E", fontSize: "0.875rem", border: "1px solid rgba(255,255,255,0.15)", cursor: "pointer" }}
